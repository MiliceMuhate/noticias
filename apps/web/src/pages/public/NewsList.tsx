...
import { articlePath, defaultTitle, SITE_NAME, useDocumentTitle } from '../../lib/seo'
...
export default function NewsList() {
  ...
  const { data: articles, isLoading, error } = useQuery(articlesQuery(lang, category, search))
  useDocumentTitle(category ? `${category} | ${SITE_NAME}` : defaultTitle(lang))
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        {(category || search) && (
          <p className="mb-6 text-sm font-medium text-delvis-mute">
            {category && (
              <>
                {t('category')} <span className="font-bold text-delvis-ink">{category}</span>
              </>
            )}
            {search && (
              <>
                {t('resultsFor')} "<span className="font-bold text-delvis-ink">{search}</span>"
              </>
            )}
          </p>
        )}
        ...
        {hero && (
          <div className="grid gap-10 pb-2 lg:grid-cols-[1.7fr_1fr]">
            <Link to={articlePath(lang, hero.slug)} className="group block">
              <ArticleImage
                src={hero.media_url}
                alt=""
              ...
              <div className="mt-4 flex items-center gap-2.5">
                <CategoryLabel>{hero.category ?? t('football')}</CategoryLabel>
              ...
              <div className="mt-4 flex items-center gap-2.5">
                ...
                <span className="text-[13px] font-semibold text-delvis-ink">{hero.author ?? t('newsroom')}</span>
              ...
              <aside className="border-delvis-line pt-2 lg:border-l lg:pl-7 lg:pt-0">
                ...
                <div className="flex flex-col">
                  {latest.map((article, i) => (
                    <Link
                      key={article.id}
                      to={articlePath(lang, article.slug)}
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="text-xs font-medium text-delvis-mute">{article.author ?? t('newsroom')}</span>
                      </div>
                    </Link>
                  ))}
                </div>
        ...
        {grid.length > 0 && (
          <div className="mt-9">
            ...
            <div className="grid gap-7 sm:grid-cols-2 lg:grid-cols-3">
              {grid.map((article) => (
                <Link key={article.id} to={articlePath(lang, article.slug)} className="group flex flex-col gap-3">
                  <ArticleImage
                    src={article.media_url}
                    alt=""
                    category={article.category}
                  ...
                  <span className="text-xs font-medium text-delvis-mute">
                    {article.author ?? t('newsroom')} · <LocalTime iso={article.published_at} format="short" />
                  </span>
                </Link>
              ))}
            </div>
